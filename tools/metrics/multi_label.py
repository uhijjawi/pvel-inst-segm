...
class MultiLabelMetric_c(BaseMetric):
    ...
    **Support** is the number of samples:
    ...
    def process(self, data_batch, data_samples: Sequence[dict]):
        ...
        for data_sample in data_samples:
            ...
            result['pred_scores'] = data_sample['pred_instances']['scores'].clone()
            num_classes = 4 #result['pred_scores'].size()[-1]

            if 'scores' in data_sample['gt_instances']:
                result['gt_scores'] = data_sample['gt_instances']['scores'].clone()
            else:
                result['gt_scores'] = label_to_onehot(data_sample['gt_instances']['labels'],
                                                     num_classes)
    ...
    def compute_metrics(self, results: List):
        ...
        target = torch.stack([res['gt_scores'] for res in results])
        pred = torch.stack([res['pred_scores'] for res in results])
        # import torch.nn.functional as F
        # max_size = max([res['pred_scores'].size(0) for res in results])
        # padded_preds = [F.pad(res['pred_scores'], (0, max_size - res['pred_scores'].size(0))) for res in results]
        # pred = torch.stack(padded_preds)
...
@METRICS.register_module()
class AveragePrecision_c(BaseMetric):
    ...
    def process(self, data_batch, data_samples: Sequence[dict]):
        ...
        for data_sample in data_samples:
            ...
            result['pred_scores'] = data_sample['pred_score'].clone()
