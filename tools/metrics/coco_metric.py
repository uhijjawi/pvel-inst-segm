### COPY THIS FILE TO MMDETECTION INSTALLATION DIRECTORY ###
### To: mmdetection/mmdet/evaluation/metrics/coco_metrics.py ###
...
class CocoMetric(BaseMetric):
    ...
    def __init__(self,
        ...
        allowed_metrics = ['z', 'segm', 'proposal', 'proposal_fast']
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        def per_class_AR_table(coco_eval, class_names=None, headers=["class", "AR"], columns=6):
            per_class_AR = {}
            ...
            if _class_names is not None:
                assert len(_class_names) == recalls.shape[1]
            else:
                _class_names = [str(i) for i in range(recalls.shape[1])]
        ...
        def per_class_AP_table(coco_eval, class_names=None, headers=["class", "AP"], columns=6):
            per_class_AP = {}
            ...
            _class_names = class_names
            if _class_names is not None:
                assert len(_class_names) == precisions.shape[2]
            else:
                _class_names = [str(i) for i in range(precisions.shape[2])]
        ...
        for metric in self.metrics:
            ...
            if metric == 'proposal':
                ...
                if self.classwise:  # Compute per-category AP
                    ...
                    info = ""
                    AP_table = per_class_AP_table(coco_eval)
                    info += "per class AP:\n" + AP_table + "\n"
                            
                    AR_table = per_class_AR_table(coco_eval)
                    info += "per class AR:\n" + AR_table + "\n"
